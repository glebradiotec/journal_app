"""Генерация XML для загрузки в ИС «Метафора» (КИАС РЦНИ), формат «Journal».

Схема почти идентична eLibrary Journal XML (see export_elibrary.py) — «Метафора»
принимает тот же формат <journal>. Отличия: titleid необязателен (Метафора
опознаёт журнал по ISSN, зарегистрированному в КИАС РЦНИ), плюс добавлен volume.
"""
from lxml import etree

import publish_titleid_store as titleid_store
from export_elibrary import _el, _individ_info


def build_metafora_xml(issue, journal_title_ru, journal_title_en=""):
    ...
    if issue.volume:
        _el(issue_el, "volume", issue.volume)
    if issue.number:
